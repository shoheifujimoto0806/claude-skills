...
- `search_conditions`: 検索条件（共通キーワード）
  - `keywords`: メインの検索キーワード
  - `broad_keywords`: 広め条件用
  - `narrow_keywords`: 絞り込み条件用
- `scout_message_base`: スカウト文面のベーステンプレート（scout-writer が生成、scout-reviewer が検証済み）
- `evaluation_framework`: 候補者評価フレームワーク（score_criteria, job_specific_criteria, bonus_factors, attention_point_rules）

検索条件は manifest.yaml の `constraints.search_filters` に基づいて媒体固有フィルタにマッピングする。
...
候補者ごとの scout_message を生成した後、scout-pm に制御を戻し段階2レビューを受ける。レビューループ自体は scout-pm が制御するため、本スキルは scout-reviewer を直接呼ばない。

フロー:
1. 着目ポイント生成 + scout_message_base のカスタマイズ → scout_message を YAML に保存
2. scout-pm が scout-reviewer の段階2レビューを実行
3. PASS → リスト追加に進む
4. FAIL → scout-pm から修正指示を受け取り、scout_message を修正して YAML を更新
...
1. `<platform>.yaml` の `list_id` を確認。値があれば既存リストを使用（ステップ4へ）
2. `list_id` が null の場合、媒体のリスト管理画面で `list_name` と一致するリストを検索
   - 見つかった場合: その list_id を YAML に記録して使用
   - 見つからなかった場合: 新規作成
3. リスト作成・特定後、`list_id` と `list_name` を `<platform>.yaml` に即時保存（チェックポイント）
4. 以降の候補者追加はこのリストに対して行う
